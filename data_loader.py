...
import lol_personal_experiment.util.collection_util as cu #경로 바꿔줌
...
class DataLoader:
    ...
    def __init__(self, dataset_dir, x_includes, x_expand=0, train_prop=0.5, valid_prop=0.5):
        ...
        print("배고파",all_segment_list)
        self.all_segment_df = pd.DataFrame(all_segment_list)
        print("구구로")
        print(self.all_segment_df)
        # segment sequence 데이터 생성 시 성능 향상을 위해 dictionary 구조 병행 사용
        # self.all_segment_dict = cu.list_to_dict(all_segment_list, itemgetter('title')) #바꿈 주석
        self.all_segment_dict = all_segment_list
    ...
    def _split_dataset(self):
        def get_subset(title):
            ...
            print("hashing볶이",hashing)
            hashing.update(title.encode())
            print("떡",hashing.update(title.encode()))
            digest = hashing.digest()
            print("digest",digest)
            hvalue = reduce(lambda x, y: x ^ y, digest) / 255
            print("나나",hvalue)
            print("너나",self.train_prop)
            print("벨리드",self.valid_prop)
            ...
            while True:
                ...
                for _, segment in batch_df.iterrows():
                    ...
                    if self.x_expand > 0:
                        ...
                        # title_segment_list = title #바꿈
                        # title_segment_list = self.all_segment_dict[title] 바꿈
                        b = []
                        # title = 'train_lol2'
                        for jak in self.all_segment_dict:
                            if title == jak['title']:
                                b.append(jak)
                        title_segment_list=b
